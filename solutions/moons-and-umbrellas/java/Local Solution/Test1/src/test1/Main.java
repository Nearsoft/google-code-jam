...
public class Main {
	...
	public static void main(String[] args) {
		...
		Case[] Cases = ReadCases(getName("test","3", "input"));
		String[] Solutions = ReadSol(getName("test","3", "output"));
		...
		for (Case cas:Cases){
			...
			String sol = solver.solve(cas, i);
			if (sol.equals(Solutions[i])) {corr+=1;}
			else {
				System.out.format("REAL: %s %n",Solutions[i]);
				System.out.format("Data: X:%d Y:%d %n",cas.X,cas.Y);
				System.out.format("Founded %s %n%n",sol);
			}
			i+=1;
		}
		System.out.format("%n%d solved correctly",corr);
	}
